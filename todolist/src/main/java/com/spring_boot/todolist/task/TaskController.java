...
import com.spring_boot.todolist.utils.Utils;
    ...
    public ResponseEntity createTask(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (taskModel.getStartAt().isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("A data de inicio / data de termino deve ser menor que a data de termino.");
        }
...
@PutMapping("/{id}")
    public ResponseEntity updateTask(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable UUID id) {
        ...
        if(task == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Task nao encontrada.");
        }

        var idUser = request.getAttribute("idUser");
        if (!task.getUserId().equals(idUser)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Usuario nao tem permissao para alterar esta task");
        }

        Utils.copyNonNullProperties(taskModel, task);
        var taskUpdate = taskRepository.save(task);

        return ResponseEntity.status(HttpStatus.OK).body(this.taskRepository.save(taskUpdate));
}
